<script>
    import {setTagsStoreAction} from "../tags.store";
    ...
    $: onTagAdd = onTagAdd || function(){};
    $: onTagRemove = onTagRemove || function(){};
    ...
    function addTag(currentTag) {
        let currentObjTags;
        // Adding a new tag when we're expecting objects and not string
        if (autoCompleteKey &&  currentTag && typeof currentTag !== 'object') {
            currentTag = {[autoCompleteKey]: currentTag};
        }
        ...
        if (typeof onTagAdd === 'function') {
            let newTag = autoComplete?.find(elem => {
                if (autoCompleteKey) {
                    return elem[autoCompleteKey] === currentTag;
                }
            ...
            // must be new here
            if (!newTag) {
                newTag = autoCompleteKey ? { [autoCompleteKey]: currentTag } : currentTag;
            }

            setTagsStoreAction({name: (typeof newTag === 'string') ? newTag : newTag.name, action: 'add'});
            onTagAdd(tags, newTag);
        }
    }

    function removeTag(i) {
        const toRemove = tags[i];
        tags.splice(i, 1);
        tags = tags;
        ...
        if (typeof onTagRemove === 'function') {
            onTagRemove(tags, toRemove);
        }

        setTagsStoreAction({name: toRemove, action: 'remove'});
    }
