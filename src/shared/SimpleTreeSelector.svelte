<script lang="ts">
  ...
  import { ArrowsPointingOut, ArrowRight, Plus, Home, Trash, PencilSquare } from 'svelte-heros-v2';
  import ProductCategoryEdit from "../lib/components/ProductCategoryEdit.svelte";
  ...
  function openCreateModal(node) {
    ...
    categoryData.parentId = node ? node.id : null;
  ...
  async function confirmCreate() {
    ...
    categoryData = { title: '', description: '', parentId: null };
  }
  ...
  async function confirmUpdate() {
    await handleUpdate(categoryData.id, categoryData);
...
<Modal bind:open={isCreateModalOpen} size="xl">
  <ProductCategoryEdit {parentCategory} on:change={(e) => categoryData = e.detail} />
  <svelte:fragment slot="footer">
...
<Modal bind:open={isUpdateModalOpen} size="xl">
  <ProductCategoryEdit id={categoryData.id} on:change={(e) => categoryData = e.detail} />
